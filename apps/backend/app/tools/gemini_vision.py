...
async def analyze_video(args: dict, state) -> dict:
    ...
    try:
        # Compress video before sending to LLM
        compressed_path = await compress_for_analysis(file_path)
        send_path = compressed_path

        if _use_files_api():
            file_part = await _upload_via_files_api(client, send_path)
        else:
            logger.info(f"Using inline bytes for video: {send_path}")
            mime_type = mimetypes.guess_type(str(send_path))[0] or "video/mp4"
            file_part = Part.from_bytes(data=send_path.read_bytes(), mime_type=mime_type)
...
async def analyze_image(args: dict, state) -> dict:
    ...
    try:
        if _use_files_api():
            file_part = await _upload_via_files_api(client, file_path)
        else:
            logger.info(f"Using inline bytes for image: {file_path}")
...
async def _upload_via_files_api(client, file_path: Path):
    """Upload a file via the Gemini Files API and wait for processing."""
    logger.info(f"Uploading to Gemini Files API: {file_path}")
    ...
    if uploaded_file.state.name != "ACTIVE":
        raise RuntimeError(f"File processing timed out after {max_wait}s. State: {uploaded_file.state.name}")

    logger.info(f"File uploaded and ready: {uploaded_file.name}")
